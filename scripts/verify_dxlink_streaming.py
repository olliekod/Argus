...
async def verify_streaming():
    ...
    from scripts.tastytrade_health_audit import _extract_chain_expirations
    expirations = _extract_chain_expirations(chain_data)
    option_symbol = None
    if expirations:
        for strike in expirations[0].get("strikes", []):
