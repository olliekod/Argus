...
Generates precise IBIT put spread trade recommendations.
...
class TradeCalculator:
    """
    Generates complete trade recommendations for IBIT put spreads.
    ...
    def __init__(self, account_size: float = 3000.0, symbol: str = "IBIT"):
        """
    ...
    def generate_recommendation(
        ...
        Args:
            btc_change_24h: BTC 24h price change (e.g., -0.05 for -5%)
            ibit_change_24h: IBIT 24h price change
        ...
        return TradeRecommendation(
            ...
            ibit_change_24h=ibit_change_24h,
    ...
    def format_telegram_alert(self, rec: TradeRecommendation) -> str:
        ...
        lines.extend([
            "*📊 Market Conditions:*",
            f"• BTC: {rec.btc_change_24h*100:+.1f}% (24h)",
            f"• {rec.symbol}: ${rec.underlying_price:.2f} ({rec.ibit_change_24h*100:+.1f}%)",
...
CONDITIONS
  BTC 24h:    {rec.btc_change_24h*100:+.1f}%
  IBIT:       ${rec.underlying_price:.2f} ({rec.ibit_change_24h*100:+.1f}%)
