...
class IVConsensusConfig:
    ...
    ring_size: int = 8
    policy: str = "prefer_dxlink"  # prefer_dxlink | winner_based | blended
    ...
    source_weights: Dict[str, float] = field(default_factory=lambda: {"dxlink": 0.7, "public": 0.3})
...
class IVConsensusEngine:
    def __init__(self, config: Optional[IVConsensusConfig] = None) -> None:
        ...
        self._disc_abs: List[float] = []
        self._disc_rel: List[float] = []
        self._disc_total = 0
        self._disc_warn = 0
        self._disc_bad = 0
